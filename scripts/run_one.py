...
def main():
    ...
    p.add_argument("--config", default="src/configs/full.yaml")
    ...
    row = {"market": args.market, "fold": args.fold,
    ...
    pd.DataFrame([row]).to_parquet(
        out / f"{args.market}_f{args.fold:02d}_{args.variant}_s{args.seed}.parquet"
    )
